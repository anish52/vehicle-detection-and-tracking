...
#INPUT_VIDEO_PATH = r'D:\Skellam\Data\input_ReID.mp4'
...
out = cv2.VideoWriter(OUTPUT_VIDEO_DIR+'\Full_Output_ReID.avi', fourcc, fps_out, (width,height))
...
def run_inference_for_video(model):
    ...
    while(True):
        ...
        with open(OUTPUT_VIDEO_DIR+"\\Full_Logs_ReID.txt", "a") as f:
        ...
        if logs:
          ...
          timer = [0, 0]
        timer_print = "Serve Time for car at 1st position : {:3d} secs".format((timer[1]-timer[0])*fps_out)
        ...
        cv2.rectangle(image_np_with_detections, (45, 70), (420, 90), (0,0,0), -1)
